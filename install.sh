...
if command -v curl >/dev/null 2>&1; then
    if curl -fsSL "$RAW/bangbot.py" -o "$BOT_DIR/bangbot.py" 2>/dev/null; then
        DOWNLOAD_OK=1
    fi
fi
...
if [ "$DOWNLOAD_OK" -eq 0 ] && command -v wget >/dev/null 2>&1; then
    ...
    if wget -q "$RAW/bangbot.py" -O "$BOT_DIR/bangbot.py" 2>/dev/null; then
        DOWNLOAD_OK=1
    fi
fi
...
FSIZE=$(wc -c < "$BOT_DIR/bangbot.py" 2>/dev/null || echo 0)
if [ "$FSIZE" -lt 10000 ]; then
    echo -e "${RED}✗ Download incomplete (file too small: ${FSIZE} bytes). Abar try koro.${RST}"
    rm -f "$BOT_DIR/bangbot.py"
    exit 1
fi

chmod +x "$BOT_DIR/bangbot.py"
echo -e "${GRN}✓ Downloaded: $BOT_DIR/bangbot.py (${FSIZE} bytes)${RST}"
